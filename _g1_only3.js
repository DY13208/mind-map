...
const log = (m) => { const line = new Date().toISOString() + ' ' + m; console.log(line); fs.appendFileSync('D:/mind-map/_g1_only3.log', line + '\n'); };
function dockerCompose(args, extra) {
  log('docker ' + args.join(' '));
  // Detach from console process group so Docker CTRL events cannot kill this Node.
  ...
  log('docker status=' + r.status);
  if (r.status) throw new Error(String(r.stderr || r.stdout || 'fail').slice(0, 400));
}
