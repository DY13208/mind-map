...
function serializeJob(job) {
  return {
    ...
    refMaps: Array.isArray(job.refMaps) ? job.refMaps.slice(0, 8) : [],
    refMapsNote: String(job.refMapsNote || '').slice(0, 80000),
    model: job.model || '',
    actor: job.actor || '',
    backend: job.backend || '',
    backendLabel: job.backendLabel || '',
...
function hydrateJob(raw) {
  ...
  return {
    ...
    refMaps: Array.isArray(raw.refMaps) ? raw.refMaps.slice(0, 8) : [],
    refMapsNote: String(raw.refMapsNote || '').slice(0, 80000),
    model: raw.model || '',
    actor: raw.actor || '台账',
    backend: raw.backend || '',
  ...
  const api = {
    async enqueue({
      ...
      const job = {
        ...
        extraNote: extraNote || '',
        refMaps: Array.isArray(refMaps) ? refMaps.slice(0, 8) : [],
        refMapsNote: String(refMapsNote || '').slice(0, 80000),
        model: model || '',
        actor: actor || '台账',
        backend: useBackend,
