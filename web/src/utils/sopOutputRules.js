...
2. 本 SOP 步骤标题里写明的输入（如「根据历史GMV数据…制定目标」）是本步骤必做，不是可忽略的「上游」。
   - 必须尽量从本脑图/本 SOP 可达数据里取历史与趋势，算出分渠道目标数字并写进产物。
   - 只有本 SOP 子树与可达数据里确实没有该输入时，才对「该字段」标「待接入」；对应步骤状态必须是未完成/阻塞，不能把整单写成「完成」。
3. 「不必考虑上游」仅指：不要因为别的 D 节点/外部系统没跑完就停手；不要把整页写成「等上游」。绝不等于可以不做本步骤要求的历史推算。
4. 禁止交付「只有结构、全是待接入、没有目标数字」的空壳，还声称已完成「制定目标」。
5. HTML：多图少字；图要展示目标结果（分渠道 GMV/实收目标、对比历史），禁止用「店铺数/分层数」冒充目标方案图。

── B. 执行完成后再做脑图诊断（这时才看全图）──
6. 独立章节「## 脑图诊断（全图）」：顺畅度 / 需补充 / 需纠错；必须改·建议改·可提升。
7. 诊断不要当成执行失败主因；执行段按步骤真实完成度说话。
...
交付硬性：
1. 只输出一个 .html，双击可开；CSS/JS 全部内联；禁止外链 CDN、字体库、图标库、占位图/假链接。
2. 业务数据集中在 window.DASHBOARD_DATA = {...}；禁止数字散落写死在 DOM/图表配置。
3. 适配桌面与移动端。
...
禁止：position:absolute 布局（尤其卡内）；外部字体/图标/图链；多文件交付；首屏大段文字墙。`
  },
