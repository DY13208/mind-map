/**
 * OpenClaw Gateway bridge for 鑹瓥鍔╃悊椤?
 * 鏈満 Node 浠?gateway-client/backend 韬唤杩為緳铏撅紝鍐嶅悜娴忚鍣ㄦ毚闇茬畝鍖?WebSocket銆?
 *
 * 娴忚鍣ㄥ崗璁紙JSON 鏂囨湰甯э級锛?
 *   鈫?{ type:'chat', id, conversationId, message }
 *   鈫?{ type:'delta', id, text }
 *   鈫?{ type:'tool', id, name, phase, detail }
 *   鈫?{ type:'done', id, text }
 *   鈫?{ type:'error', id, message }
 *   鈫?{ type:'status', ok, connected }
 */
...
function onGatewayEvent(evt) {
  ...
  // agent 浜嬩欢锛歛ssistant / tool / lifecycle
...
function connectGateway() {
  ...
  gateway = new GatewayClient({
    ...
    // 榛樿鍗充负 gateway-client / backend锛坙oopback 鍙渷鐣?device锛?
...
async function handleChat(ws, msg) {
  ...
  if (!message) {
    sendBrowser(ws, { type: 'error', id: chatId, message: '绌烘秷鎭? })
    return
  }
  try {
    await ensureGateway()
    ...
    sendBrowser(ws, {
      type: 'error',
      id: chatId,
      message: (err && err.message) || '鏃犳硶杩炴帴 OpenClaw Gateway'
    })
    return
  }
